...
def perm_check(request, *args, **kwargs):
    ...
    if url_name:
        ...
        url_args_list = ','.join(url_args_list) if len(url_args_list)!=0 else None
        print(url_name,url_method,'l:',url_args_list,request.user)
        # if url_args_list='':
        url_method=1 if url_method=='GET' else 2
        #操作数据库
        get_perm = models.Permission.objects.filter(Q(url=url_name) and Q(per_method=url_method) and Q(argument_list=url_args_list))
        # get_perm=models.Permission.objects.filter(Q(url=url_name) and Q(per_method=1)and Q(argument_list=url_args_list))
        print(get_perm)
        if get_perm:
            print('答应')
            for i in get_perm:
                perm_name = i.name
                perm_str = 'polls.%s' % perm_name
...
def check_permission(fun):    #定义一个装饰器，在views中应用
    def wapper(request, *args, **kwargs):
        ...
        return render(request, 'polls/403.html', locals())
    return wapper